...
gdalProjRaster <- function(img,target,outname=NULL,method="near",wm=1000,nt=1,ot="Float32",NAflag=-9999){
  ...
  if(Sys.info()[[4]]=="COPERNICUS"){
    library(gdalUtils)
    gdalwarp(fname,outname,t_srs=t_srs,te=te,ts=ts,r="near",output_Raster=F)
